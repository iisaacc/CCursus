...
/*   Updated: 2023/05/13 11:40:11 by isporras         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	bf[4096][BUFFER_SIZE + 1];
	char		*line;
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (ft_bzero(bf[fd], BUFFER_SIZE), NULL);
	line = NULL;
	if (bf[fd][0] == 0)
		return (ft_bzero(bf[fd], BUFFER_SIZE), ft_fill_line(bf[fd], fd, line));
	else
		return (ft_get_buffer_line(bf[fd], line, fd));
}
